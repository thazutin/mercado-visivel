...
async function fetchHealth(): Promise<HealthResponse | null> {
  ...
  // (viro-xxx.vercel.app) que pode ter routing diferente do alias de produção.
  ...
  try {
    ...
    if (!res.ok && res.status !== 503) {
      // 503 é esperado quando ok=false; qualquer outro status é erro real
      console.error(`[HealthReport] /api/health returned HTTP ${res.status}`);
    }
    const data = (await res.json()) as HealthResponse;
    console.log(`[HealthReport] health snapshot: ok=${data.ok}, paused=${data.paused}, ${data.checks?.length} checks`);
    return data;
  } catch (err) {
